...
export class Bank {
    constructor(name, HTTPport, UDPport, gRPCport) {
        this.name = name;
        this.portfolio = 0;
    ...
    startServer() {
        ...
        server.on("message", (msg, rinfo) => {
            ...
            this.receiveData(
                parsedData.wertpapier,
                parsedData.count,
                parsedData.preis
            );
            const responseBuffer = Buffer.from(
                `Received from Boerse 1234: ${rinfo.address}, on port ${rinfo.port} ${parsedData.wertpapier.kurzel}, ${parsedData.count}`
            );
        ...
        server.on("listening", () => {
            ...
            console.log(
                `Bank server listening on ${address.address}:${address.port}`
            );
        });
        //server start listening for incoming data gram ipAddress:port
        server.bind(this.UDPport, this.ipAddress);
    }
    ...
    receiveData(Wertpapier, count, preis) {
        ...
        console.log(
            `Received data from Boerse: ${Wertpapier.kurzel}, count: ${count}`
        );
        console.log(this.calculatePortfolio());
    }
    ...
    startHttpServer() {
        const server = net.createServer((socket) => {
            ...
            //error handling when client close connection
    ...
    handleGetRequest(socket, path) {
        ...
        if (path === "/bank/portfolio") {
            ...
            if (wertpapier) {
                ...
                this.sendJsonResponse(socket, {
                    success: false,
                    message: "Invalid Wertpapier Kurzel",
                });
            }
        } else {
    ...
    sendInvalidMethodResponse(socket) {
        const response =
            "HTTP/1.1 error 405 method not allowed \r\nContent: 0\r\n\r\n";
    ...
    startGrpcServer() {
        const bankService = new BankService(this);
        const server = new grpc.Server();
        const port = this.gRPCport; 
        server.addService(bankProto.BankService.service, {
            CalculatePortfolio: bankService.grpcCalculatePortfolio.bind(bankService),
            addWertpapier: bankService.grpcAddWertpapier.bind(bankService),
        });
    
        server.bindAsync(`0.0.0.0:${port}`, grpc.ServerCredentials.createInsecure(), (err, port) => {
            if (err) {
                console.error(`Failed to start the gRPC server: ${err.message}`);
            } else {
                console.log(`Bank gRPC server listening on port ${port}`);
                server.start();
            }
        });
    }
...
export const firstBank = new Bank("firstBank", 8080, 3000, 50051);
export const secondBank = new Bank("secondBank", 8081, 3001, 50052);
